...
// 导入 验证码工具
import { genCaptcha, verifyCaptcha } from '../../../utils/captcha.js'
import { nanoid } from "nanoid";
import { addLog, addSuccessResult, addErrorResult } from '../../../utils/reactLogUtil.js'

import { loginUserMap, captchaMap } from "../RedisMock.js";

const router = express.Router();

// 获取验证码
router.get('/getCaptcha', (req, res) => {
    const { uuid, captchaData } = genCaptcha()
    captchaMap.set(uuid, {
        timestamp: Date.now(),
        text: captchaData.text,
    });
    res.send({ code: 200, data: { uuid, img: captchaData.data }, message: "success" });
})
...
router.post("/login", async (req, res) => {
    const logId = nanoid()
    try {
        const { body: { username, password, uuid, captcha } } = req

        await addLog({ id: logId, module: "用户模块", type: '登录', username: username, req })

        const verifyInfo = verifyCaptcha(uuid, captcha);

        // 校验验证码
        if (!verifyInfo.flag) {
            addErrorResult({ id: logId, result: verifyInfo.msg })
            return res.send({ code: 500, data: null, message: verifyInfo.msg });
        }

        const [data] = await sql.query(
            "select * from react_user where username = ? and password = ? and is_delete = 0",
            [username, password]
        );
        if (data.length === 0) {
        ...
        const token = `Bearer ${jwt.sign(userInfo, config.secretKey, { expiresIn: config.expiresIn })}`
        loginUserMap.set(userInfo.id, token)
...
router.get("/getPermissionList", async (req, res) => {
    ...
    try {
        ...
        if (Number(jwtInfo['is_admin']) === 1) {
            sqlStr = "select id, pid, fullname, path, sort, icon, permission_code, component, type, status, keep_alive, is_link, link_url from react_menu where is_delete = 0"
        } else {
            // 普通用户
            // 获取 用户ID 和 用户角色ID
            const id = jwtInfo.id || ""
            ...
            // 2. 通过 角色ID 查询 菜单权限ID
            const [permissionData] = await sql.query('select permissions from react_role where id in (?)', [userRoleArr])
            let permissions = []
            ...
            // 3. 通过菜单ID, 查询用户菜单
            sqlStr = `select id, pid, fullname, path, sort, icon, permission_code, component, type, status, keep_alive, is_link, link_url from react_menu where is_delete = 0 and id in (${permissions})`
        }
        ...
        if (Number(jwtInfo['is_admin']) === 1) {
            ...
            persList = data?.filter(v => v.type === "B")?.map(v => v['permission_code'])
        }

        addSuccessResult({ id: logId })
        res.send({ code: 200, data: { persList, routerList: list }, message: "success" });
    } catch (err) {
        addErrorResult({ id: logId, result: JSON.stringify(err) })
    }
});
...
router.post("/list", async (req, res) => {
    ...
    try {
        ...
        let totalSql = "select count(id) as total from react_user where is_delete = 0 and is_admin = 0"
        let sqlStr = "select id, nickname, username, email, remark, status from react_user where is_delete = 0 and is_admin = 0"

        if (Number(jwtInfo['is_admin']) === 1) {
            totalSql = "select count(id) as total from react_user where is_delete = 0"
            sqlStr = "select id, nickname, username, email, remark, status from react_user where is_delete = 0"
        }
...
router.post("/add", async (req, res) => {
    ...
    try {
        ...
        const [user] = await sql.query(
            "select id from react_user where username = ? and is_delete = 0",
            [username]
        );
        if (user.length !== 0) {
        ...
        const [data] = await sql.query(
            "insert into react_user (nickname, username, password, email, remark, status) values (?,?,?,?,?,?)",
...
router.get("/detail", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "select id, nickname, username, email, remark, status from react_user where id = ? and is_delete = 0",
            [id]
        );
...
router.post("/edit", async (req, res) => {
    ...
    try {
        ...
        let [data] = await sql.query(
            "select id, is_admin from react_user where id = ? and is_delete = 0 and is_admin = 1",
            [id]
        );
        if (data.length) {
        ...
        [data] = await sql.query(
            "select id from react_user where username = ? and id != ? and is_delete = 0",
            [username, id]
        );
        if (data.length !== 0) {
            addErrorResult({ id: logId, result: "用户名已存在" })
        ...
        [data] = await sql.query(
            "update react_user set nickname = ?, username = ?, email = ?, remark = ?, status = ? where id = ? and is_delete = 0 and is_admin = 0",
...
router.post("/status", async (req, res) => {
    ...
    try {
        ...
        [data] = await sql.query(
            "update react_user set status = ? where id = ? and is_delete = 0",
            [status, id]
        );
        if (data.affectedRows === 1) {
            addSuccessResult({ id: logId })
...
router.post("/delete", async (req, res) => {
    ...
    try {
        ...
        [data] = await sql.query(
            "update react_user set is_delete = 1 where id = ? and is_admin = 0",
            [id]
        );
...
router.post("/batchDelete", async (req, res) => {
    ...
    try {
        ...
        // 查询是否为 超级管理员
        let [data] = await sql.query(
            "select id, is_admin from react_user where id in (?) and is_delete = 0 and is_admin = 1",
            [body]
        );
        if (data.length) {
            addErrorResult({ id: logId, result: "无权限" })
        ...
        [data] = await sql.query(
            "update react_user set is_delete = 1 where id in (?) and id != 1 and is_admin = 0",
            [body]
        );
...
router.get("/getRoles", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "select id,roles from react_user where id = ? and is_delete = 0",
            [id]
        );
...
router.post("/editRole", async (req, res) => {
    ...
    try {
        ...
        [data] = await sql.query(
            "update react_user set roles = ? where id = ? and is_admin = 0",
