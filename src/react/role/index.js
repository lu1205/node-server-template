...
import { findParentMenusById } from '../../../utils/index.js'
...
router.get("/all", async (req, res) => {
    ...
    try {
        ...
        const sqlStr = "select id, fullname from react_role where is_delete = 0 and status = 0"
...
router.post("/list", async (req, res) => {
    ...
    try {
        ...
        let sqlStr = "select id, fullname, remark, permissions, status from react_role where is_delete = 0";
        if (fullname) {
            sqlStr += ` AND fullname LIKE '%${fullname}%'`
        }

        const [roleData] = await sql.query(sqlStr)
        const total = roleData.length
        // 分页
        let list = roleData.splice((pageNum * 1 - 1) * pageSize, pageSize * 1)


        const [menuData] = await sql.query("select id, pid, fullname, type from react_menu where is_delete = 0")
        list = list?.map(role => {
            const permissionsName = role.permissions ? role.permissions?.split(',')?.map(v => {
                return findParentMenusById(Number(v), menuData)?.filter(_ => _.type === "M")?.map(_ => _.fullname)
            }) : []

            return {
                ...role,
                permissionsName: Array.from(new Set(permissionsName.flat()))?.join(',')
            }
        })
...
router.post("/add", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "insert into react_role (fullname,remark,status) values (?,?,?)",
...
router.get("/detail", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "select id, fullname, remark, permissions, status from react_role where id = ? and is_delete = 0",
            [id]
        );
...
router.get("/permissionDetail", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "select id, permissions from react_role where id = ? and is_delete = 0",
            [id]
        );
...
router.post("/edit", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "update react_role set fullname = ?, remark = ?, status = ? where id = ? and is_delete = 0",
...
router.post("/status", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "update react_role set status = ? where id = ? and is_delete = 0",
            [status, id]
        );
        if (data.affectedRows === 1) {
            addSuccessResult({ id: logId })
...
router.post("/delete", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "update react_role set is_delete = 1 where id = ?",
            [id]
        );
...
router.post("/batchDelete", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "update react_role set is_delete = 1 where id in (?)",
            [body]
        );
...
router.post("/editPermissions", async (req, res) => {
    ...
    try {
        ...
        const [data] = await sql.query(
            "update react_role set permissions = ? where id = ?",
