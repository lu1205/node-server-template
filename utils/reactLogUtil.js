...
export const addLog = async (data) => {
  ...
  try {
    ...
    await sql.query(
      'insert into react_logs (ip, uuid, module, type, username, method, path, query, params, body, create_time, browser, os) values (?,?,?,?,?,?,?,?,?,?,?,?,?)',
...
export const addSuccessResult = async (data) => {
  try {
    ...
    await sql.query("update react_logs set result = ?, update_time = ?, result_type = 1 where uuid = ?", [result, updateDate, id])
  } catch (e) {
    console.log(e);
  }
}
...
export const addErrorResult = async (data) => {
  try {
    ...
    await sql.query("update react_logs set result = ?, update_time = ?, result_type = 0 where uuid = ?", [result, updateDate, id])
  } catch (e) {
    console.log(e);
  }
}